...
class EventApp {
	...
	createNewEvent(opt) {
		...
		if (this.#events.has(opt.eventID)) {
	...
	getCurrentEvent() {
		const now = new Date();
		const events = [];
		for (const eventObj of this.#events.values()) if (eventObj.details.passcodeStartTime < now && now < eventObj.details.passcodeEndTime) events.push(eventObj);
		return events;
	}
